...
void LevelLoader::LoadLevel(sol::state& lua, const std::unique_ptr<Registry>& registry, const std::unique_ptr<AssetStore>& assetStore, SDL_Renderer* renderer, const int levelNumber) {
    sol::load_result script = lua.load_file("./assets/scripts/Level" + std::to_string(levelNumber) + ".lua");
    
    // checks the syntax of our script but does not execute the script
    if (!script.valid()) {
    ...
    // execte the script using the Sol state 
    ...
    while (true) {
        ...
        if (assetType == "texture") {
            ...
            Logger::Log("A new texture asset loaded to the asset store, id: " + assetId);
        }

        if (assetType == "font") {
            assetStore->AddFont(assetId, asset["file"], asset["font_size"]);
            Logger::Log("A new font asset loaded to the asset store, id: " + assetId);
        }
